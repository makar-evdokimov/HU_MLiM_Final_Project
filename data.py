...
to construct the input features for the predictive model.
...
def co_occurrences_sparse(x, variable_basket="basket", variable_product="product"):
    """Produces co-occurrence matrix for products/categories given dataset of purchase histories"""
...
def create_discount_features(dataset: pd.DataFrame, subst_list: list, compl_list: list):
    """Creates discount-based features used as an input fpr the model
    ...
    dataset.drop('disc_self_cat', axis=1, inplace=True)
...
def create_other_features(train_set: pd.DataFrame, baskets: pd.DataFrame):
    ...
    Returns dataset of input features with index week-shopper-product

    """

    baskets.sort_values(by=['shopper', 'product', 'week'], inplace=True)

    # feature 'number of weeks since the last purchase' is created
    baskets['time_since_last_purchase_of_product'] = 0
    baskets['time_since_last_purchase_of_product'] = np.where((baskets['product'] == baskets['product'].shift()) &
                                                               (baskets['shopper'] == baskets['shopper'].shift()),
                                                               baskets['week'] - baskets['week'].shift(), 0)

    train_set = train_set.merge(baskets[['week', 'product', 'shopper', 'time_since_last_purchase_of_product']],
                                how='left', on=['week', 'product', 'shopper'])

    train_set['y_inv'] = 0
    train_set['y_inv'] = np.where(train_set['y'] == 0, 1, 0)

    train_set.sort_values(by=['shopper', 'product', 'week'], inplace=True)

    train_set['n_prev_purch'] = train_set.groupby(['shopper', 'product']).cumsum()['y']
    train_set['spec_number'] = train_set.groupby(['shopper', 'product', 'n_prev_purch']).cumsum()['y_inv']

    train_set.loc[np.isnan(train_set['time_since_last_purchase_of_product']),
                  'time_since_last_purchase_of_product'] = train_set['spec_number']
    train_set.loc[(train_set['n_prev_purch'] == 0), 'time_since_last_purchase_of_product'] = 0

    train_set.drop(['y_inv', 'spec_number', 'n_prev_purch'], axis=1, inplace=True)

    # feature 'redemption rate' is created
    baskets['coupon_used'] = 0
    baskets.loc[baskets['discount'] > 0, 'coupon_used'] = 1

    train_set = train_set.merge(baskets[['week', 'shopper', 'product', 'coupon_used']], how='left',
    ...
    train_set['coupon_previously_offered'] = train_set.groupby(['shopper', 'product']).cumsum()['coupon_offered']
    train_set['coupon_previously_used'] = train_set.groupby(['shopper', 'product']).cumsum()['coupon_used']
    ...
    train_set['avg_purch_freq'] = train_set.groupby(['shopper', 'product']).cumsum()[
                                      'y'] / (train_set['week'] + 1)
...
def create_prediction_set(train_set: pd.DataFrame):
    ...
    Returns the dataset
    The function must be applied to the corresponding training dataset
    """

    prediction_set = train_set[train_set['week'] == max(train_set['week'])]

    prediction_set['week'] += 1
    prediction_set['time_since_last_purchase_of_product'] = np.where(
        prediction_set['time_since_last_purchase_of_product'] == 0,
        0, prediction_set['time_since_last_purchase_of_product'] + 1)

    # important - if the product was bought on week 89
    # then on week 90 "number of weeks since the last purchase" is 1
    prediction_set.loc[(prediction_set['y'] == 1), 'time_since_last_purchase_of_product'] = 1

    prediction_set.drop(['redemption_rate_shift', 'avg_purch_freq_shift',  'discount', 'disc_other_in_cat',
                         'disc_subst', 'disc_compl', 'y'], axis=1, inplace=True)
...
if __name__ == "__main__":
    ...
    train_set['y'] = np.where(np.isnan(train_set['price']), 0, 1)
    ...
    try:
        tsne_cat = pd.read_csv(f"{path_processed}/tsne_cat.csv")
        ...
        tsne_cat = pd.DataFrame(data={'x': new_values.T[0], 'y': new_values.T[1], 'product': labels})
        tsne_cat['product'] = tsne_cat['product'].astype(int)

        tsne_cat = tsne_cat.sort_values(by='product').reset_index(drop=True)

        # number of product categories (25) has been chosen manually
        # based on the visual analysis of the product map
        ...
        xy = tsne_cat[['x', 'y']].values.astype('double')
        ...
        for ix, iy in indices:
            ...
            tsne_cat['category'] = cluster_labels

            tsne_cat.to_csv(f"{path_processed}/tsne_cat.csv", index=False)

    n_cat = config['data']['n_cat']

    train_set = train_set.merge(tsne_cat[['product', 'category']], how='left', on='product')
    baskets = baskets.merge(tsne_cat[['product', 'category']], how='left', on='product')

    try:
        with open(f"{path_processed}/cat_subst.txt", 'rb') as fp:
            cat_subst = pickle.load(fp)
        with open(f"{path_processed}/cat_compl.txt", 'rb') as fp:
    ...
    # the function is used to create dataset
    # with all necessary features
    train_set = create_other_features(train_set=train_set, baskets=baskets)
    train_set.sort_values(by=['week', 'shopper', 'product'], inplace=True)

    # input and labels for model
    y = pd.DataFrame(train_set['y'])
    order_features = ['max_price', 'discount', 'disc_other_in_cat', 'disc_subst', 'disc_compl',
                      'time_since_last_purchase_of_product', 'avg_purch_freq_shift', 'redemption_rate_shift']
    X = train_set[order_features]

    # set to make predictions for week 90 is created
    prediction_set = create_prediction_set(train_set)

    # all necessary processed data is saved
    train_set.to_parquet(f"{path_processed}/train_set.pt")
    prediction_set.to_parquet(f"{path_processed}/prediction_set.pt")
    X.to_parquet(f"{path_processed}/x.pt")
    y.to_parquet(f"{path_processed}/y.pt")
