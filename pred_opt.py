...
if __name__ == "__main__":
    ...
    prediction_set = pd.read_parquet(f"{path_data}/processed/prediction_set.pt")
    prediction_set = prediction_set.reset_index(drop=True)
    lightgbm_classifier = joblib.load(f"{path_data}/models/lightgbm_model.pkl")
    ...
    def revenue_per_shopper_pred(coupons, pred_set):
        """Uses the built two-level model and two prediction datasets

        to estimate the weekly revenue generated by one shopper
        """

        pred_set_b = pred_set.copy()
        pred_set_b = pred_set_b.reset_index(drop=True)
        coupons = coupons.reset_index(drop=True)
        pred_set_b = pd.concat([pred_set_b, coupons], axis=1)
        pred_set_b = create_discount_feature_pred(pred_set_b)
        X_pred = pred_set_b[order_features]
        pred_set_b['purch_prob'] = lightgbm_classifier.predict_proba(X_pred)[:, 1]
        revenue = sum(pred_set_b['max_price'] * (1 - pred_set_b['discount']) * pred_set_b['purch_prob'])
    ...
    def objective(trial):
        ...
        This function exploits the convenience of the structure
        of the artificial data (25 categories with 10 products in each)

        However, it can be elaborated on to make it applicable to the real-world data
        with some more stochastic implicit categorical structure of the product range
        """
        ...
        revenue = revenue_per_shopper_pred(coupons=pred_index['discount'], pred_set=pred_set)
    ...
    prediction_set_index = prediction_set[['week', 'shopper', 'product', 'category']]
    ...
    n_prod = max(prediction_set['product']) + 1
    n_cat = max(prediction_set['category']) + 1
    n_shoppers = max(prediction_set['shopper']) + 1

    # number of iterations algorithms perform per shopper
    n_iter = config['optimizer']['n_iter']

    # list of discounts that can be offered to shoppers
    ...
    for i in range(n_shoppers):
        base_revenue_per_shopper.append(
            revenue_per_shopper_pred(coupons=prediction_set_index[prediction_set_index.shopper == i]['discount'],
                                     pred_set=prediction_set[prediction_set.shopper == i]))
    ...
    for i in range(len(coupons_per_shopper_optuna), n_shoppers):
        ...
        pred_set = prediction_set[prediction_set.shopper == i].copy()
